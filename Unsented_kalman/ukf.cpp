...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
  ...
  if(!is_initialized_){
   ...
   if (cnt==497){
    cout<<"NIS value is :"<<nis/cnt*100<<endl;
   }
}
/**
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  MatrixXd Xsig_aug=CreateXsigmaAug();
...
//NIS for Lidar
  double nis_val=0.0;
  nis_val=(z_diff.transpose())*S.inverse()*z_diff;
  cnt+=1;
  if (nis_val<5.991){
    nis+=1.0;
  }
...
//NIS for Radar
  double nis_val=0.0;
  //cout<<(z_diff)*S.inverse()*(z_diff.transpose());
  nis_val=(z_diff.transpose())*S.inverse()*(z_diff);
  cnt+=1;
  if (nis_val<7.815){
    nis+=1;
...
// ###################### converting into measurement space  ###############################
